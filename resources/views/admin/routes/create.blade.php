    ...
    <form action="{{ route('admin.routes.store') }}" method="POST">
      ...
      <div class="mb-3">
        ...
        <select name="origin_station_id" class="form-select @error('origin_station_id') is-invalid @enderror" required>
          ...
          <option value="{{ $s->id }}" {{ old('origin_station_id') == $s->id ? 'selected' : '' }}>{{ $s->name }} ({{ $s->code }})</option>
          @endforeach
        </select>
      ...
      <div class="mb-3">
        ...
        <select name="destination_station_id" class="form-select @error('destination_station_id') is-invalid @enderror" required>
          <option value="">Select Station</option>
          @foreach ($stations as $s)
          <option value="{{ $s->id }}" {{ old('destination_station_id') == $s->id ? 'selected' : '' }}>{{ $s->name }} ({{ $s->code }})</option>
          @endforeach
        </select>
      ...
      <div class="mb-3">
        ...
        <input type="number" name="distance_km" class="form-control @error('distance_km') is-invalid @enderror" value="{{ old('distance_km') }}" step="0.01" min="0">
        @error('distance_km')<div class="invalid-feedback">{{ $message }}</div>@enderror
      </div>
      <div class="mb-3">
        <label class="form-label">Estimated Duration (minutes)</label>
        <input type="number" name="estimated_duration" class="form-control @error('estimated_duration') is-invalid @enderror" value="{{ old('estimated_duration') }}" min="1">
